...
// Code retrieved from https://stackoverflow.com/questions/3982320/convert-integer-to-string-without-access-to-libraries
void reverse(char s[])
{
    int i, j;
    char c;
...
int create_user( char *user, int id, char *pass)
{
    ...
    if ( (fd = open(passwd_file, O_RDWR)) < 0 ){
		printf(1, "No se pudo añadir un nuevo usuario\n");
		return 0;
    }
    else{
		read( fd, readbuf, sizeof(readbuf) );
		if ( ( w = write(fd, new_user, strlen(new_user) )) > 0 ){
			printf(1,"Usuario añadido exitosamente\n");
		};
...
{
    if(argc != 3){
        printf(2, "Escribe el usuario y la contraseña\n");
        exit();
    }
    ...
    // Check whether user exists on each line of file
    while ( ( n = readLine(fd_rd, record, sizeof(record)) ) > 0 )
    {
        if ( user_exist(record, argv[1]) )
        {
            printf(2, "El ombre de usuario ya está en uso, escriba uno diferente\n");
            exit();
        }
        current_id = get_current_id( record );
    }
    close(fd_rd);
    //printf(2, "curr id: %d", current_id);
